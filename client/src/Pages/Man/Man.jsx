...
const Man = () => {
  ...
  useEffect(() => {
    const fetchManPosts = async () => {
      try {
        ...
        if (res.status === 200) {
          let manPostsArray = res.data.posts.filter(
            (post) => post.gender === "man"
          );
    
          // Kategoriye göre filtreleme
          if (category !== "all products") {
           manPostsArray = manPostsArray.filter(
  ...
  const filterColor = (color) => {
    if (color === "All") {
      ...
      const filteredManPosts = posts.filter(
        (post) => post.gender === "man"
      );
      setManPosts(filteredManPosts);
    } else {
      setSelectedColor(color);
  
      // Filter posts based on gender and selected color
      const tempArray = posts.filter(
        (post) => post.gender === "man" && post.color === color
      );
      
      setManPosts(tempArray);
    }
  };
